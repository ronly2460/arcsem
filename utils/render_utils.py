...
def generate_ellipse_path(
    poses: np.ndarray, n_frames: int = 120, const_speed: bool = True, z_variation: float = 0.5, z_phase: float = 0.0
) -> np.ndarray:

    # poses = poses[[28, 16, 15, 14, 13, 12, 11, 10, 9, 31, 0, 1, 30, 2, 3, 4, 5, 6, 7, 8]]

    poses = poses[[13, 12, 11, 10, 9, 0, 1, 2, 3]]
    print(poses[:, :3, 3].shape)
    print(
        np.mean(poses[:, :3, 3], axis=0),
        np.max(poses[:, :3, 3], axis=0),
        np.min(poses[:, :3, 3], axis=0),
        np.std(poses[:, :3, 3], axis=0),
    )
    # poses = poses[:, [2, 0, 1], :]
    ...
    # offset = np.array([center[1], center[0], 0])
    ...
    z_high = np.percentile((poses[:, :3, 3]), 20, axis=0)
    ...
    #             low[1] - 200 + (high - low)[1] * (np.sin(theta) * 0.5 + 0.5),
    ...
    def get_positions(theta):
        ...
        return np.stack(
            [
                ...
                low[0] + (high - low)[0] * (np.sin(theta) * 0.5 + 0.5),
                # low[1] + (high - low)[1] * (np.sin(theta) * 0.5 + 0.5),
                z_variation * (z_low[2] + 0 * (z_high - z_low)[2] * (np.cos(theta + 2 * np.pi * z_phase) * 0.5 + 0.5)),
            ],
            -1,
        )
    ...
    positions = []
    for theta in np.linspace(0.0, 2.0 * np.pi, n_frames + 1)[:-1]:
        c = np.dot(poses[0, :3, :4], np.array([np.cos(theta), -np.sin(theta), -np.sin(theta * 0.5), 1.0]) * 50)
        positions.append(c)
    ...
    # positions[:, 0] = positions[:, 0] - 300

    return np.stack([viewmatrix(p - center, up, p) for p in positions])


def generate_fancy_path(
    ...
    # Optional height variation need not be symmetric
    z_low = np.percentile((poses[:, :3, 3]), 30, axis=0)
    z_high = np.percentile((poses[:, :3, 3]), 60, axis=0)
    ...
    # Set path's up vector to axis closest to average of input pose up vectors.
    # select only lateral views for up vector.
    avg_up = poses[:, :3, 2].mean(0)
    avg_up = avg_up / np.linalg.norm(avg_up)
...
def generate_path(viewpoint_cameras, n_frames=480):
    ...
    for c2w in new_poses:
        ...
        cam = copy.deepcopy(viewpoint_cameras[5])
...
# def generate_path(viewpoint_cameras, n_frames=480, is_fancy=True):
#     c2ws = np.array(
#         [np.linalg.inv(np.asarray((cam.world_view_transform.T).cpu().numpy())) for cam in viewpoint_cameras]
#     )
#     pose = c2ws[:, :3, :] @ np.diag([1, -1, -1, 1])

#     poses = pose[[28, 16, 15, 14, 13, 12, 11, 10, 9, 31, 0, 1, 30, 2, 3, 4, 5, 6, 7, 8]]
...
#     positions = []
#     for theta in np.linspace(0.0, 2.0 * np.pi, 100)[:-1]:
#         x = np.cos(theta) * 0.5 + 0.5
#     #   position = np.dot(poses[0, :3,:4], np.array([np.cos(theta), -np.sin(theta), -np.sin(theta*0.5), 1.]) * 0.7)
...
#     new_poses = np.stack([viewmatrix_(position - center, up, position) for p in positions])

#     print(new_poses.shape)
#     #
#     # # generate new poses
#     # if is_fancy:
#     #   print("Generating fancy path")
#     #   new_poses = generate_fancy_path(poses=pose, n_frames=n_frames)
#     # else:
#     #   pose_recenter, colmap_to_world_transform = transform_poses_pca(pose)
#     #   new_poses = generate_ellipse_path(poses=pose_recenter, n_frames=n_frames)
#     #   new_poses = np.linalg.inv(colmap_to_world_transform) @ pad_poses(new_poses)
