...
def get_patch_shuf_conv(patch_size=4):
    conv_layer = torch.nn.Conv2d(3, 3 * patch_size * patch_size, kernel_size=patch_size, stride=patch_size, bias=False)
...
def nn_color_patch(top_color_centers, outputs, relation, vis=False):
    ...
    for _ in range(b):
        ...
        for center in range(color_info.shape[0]):
            ...
            temp_loss = torch.min(
                temp_loss, torch.sum(torch.nn.functional.mse_loss(temp_centorid, conv_res[0], reduce=False), dim=1)
            )
...
def get_nn_feat_relation_thre(tmpl, a):
    ...
    # ic(a_flat.shape, tmpl_flat.shape)
...
class NNFMLoss(torch.nn.Module):
        ...
        if not hasattr(self, "styl_feats_all"):
            ...
            with torch.no_grad():
                for tmpl_sty, tmpl_img in zip(tmpl_stys, tmpl_imgs):
                    # ic(tmpl_sty.shape, tmpl_img.shape)
    ...
    def forward(
        ...
        if "tcm_loss" in loss_names and "online_tmp_loss" in loss_names:
            self.tcm_flag = False
            coarse_style_flat = []
            down_fact = 16  # TODO: check
            for tmpl_sty in self.tmpl_stys:
                h_sty, w_sty = tmpl_sty.shape[:2]
                coarse_style_flat.append(
                    F.interpolate(
                        tmpl_sty.unsqueeze(0).permute(0, 3, 1, 2),
            ...
            coarse_out_flat = F.interpolate(
                outputs, (h_sty // down_fact, w_sty // down_fact), mode="bilinear", antialias=True, align_corners=True
            ).view(1, 3, -1)
        ...
        for block in blocks[:-1]:
            ...
            if "content_loss" in loss_names:
                ...
                # x_feats: features, rendered results
                # content_feats: features, dset.rays.gt
                loss_dict["content_loss"] += torch.mean((content_feats - x_feats) ** 2)

            # if "grayscale_loss" in loss_names:
            #     # outputs: pred, contents: gt
            #     contents = torchvision.transforms.functional.rgb_to_grayscale(contents, num_output_channels=1)
            #     outputs = torchvision.transforms.functional.rgb_to_grayscale(outputs, num_output_channels=1)
            #     # make the graysale
            #     loss_dict["grayscale_loss"] = F.mse_loss(contents, outputs)

            # if "depth_loss" in loss_names:
            #     # take L2 Norm on the diff of the depts.
            #     loss_dict["depth_loss"] = F.mse_loss(depths_pred, depths_gt) / (1920 * 2800 * 3)
            #     # loss_dict["depth_loss"] = F.mse_loss(depths_pred, depths_gt)

            if "tcm_loss" in loss_names:
                tmpl_feats = torch.cat([self.tmpl_feats_all[ix_map[ix]] for ix in layers], 1)
                target_feats, _ = nn_feat_replace_cond(tmpl_feats, content_feats, s_feats)
            ...
            if "color_patch" in loss_names:
                ...
                if "online_tmp_loss" not in loss_names:
                    # ic(content_feats_all[ix_map[layers_last[0]]].shape)
                    ...
                    # ic(relation_last.shape, tmpl_feats_last.shape, content_feats_last.shape)
                else:
                    content_feats_last = torch.cat([x_feats_all[ix_map[ix]] for ix in layers_last], 1)
                    relation_last = get_nn_feat_relation_thre(tmpl_feats_last, content_feats_last).repeat(1, 3, 1)

                # ic(relation_last.shape, coarse_style_flat.shape, coarse_out_flat.shape)
